...
bool InstCombiner::shouldChangeType(unsigned FromWidth,
  ...
  if (FromLegal && !ToLegal) {
    PassPrediction::PassPeeper(30); // if
    return false;
  }
  ...
  if (!FromLegal && !ToLegal && ToWidth > FromWidth) {
    PassPrediction::PassPeeper(31); // if
    return false;
  }
...
static bool MaintainNoSignedWrap(BinaryOperator &I, Value *B, Value *C) {
  ...
  if (!OBO || !OBO->hasNoSignedWrap()) {
    PassPrediction::PassPeeper(32); // if
    return false;
  }
  ...
  if (Opcode != Instruction::Add && Opcode != Instruction::Sub) {
    PassPrediction::PassPeeper(33); // if
    return false;
  }

  const APInt *BVal, *CVal;
  if (!match(B, m_APInt(BVal)) || !match(C, m_APInt(CVal))) {
    PassPrediction::PassPeeper(34); // if
    return false;
  }

  bool Overflow = false;
  if (Opcode == Instruction::Add) {
    PassPrediction::PassPeeper(35); // if
    (void)BVal->sadd_ov(*CVal, Overflow);
  } else {
    PassPrediction::PassPeeper(36); // else
...
static void ClearSubclassDataAfterReassociation(BinaryOperator &I) {
  ...
  if (!FPMO) {
    PassPrediction::PassPeeper(37); // if
...
static bool simplifyAssocCastAssoc(BinaryOperator *BinOp1) {
  ...
  if (!Cast || !Cast->hasOneUse()) {
    PassPrediction::PassPeeper(38); // if
    return false;
  }
  ...
  if (!BinOp2 || !BinOp2->hasOneUse() || BinOp2->getOpcode() != AssocOpcode) {
    PassPrediction::PassPeeper(41); // if
    return false;
  }
  ...
  if (!match(BinOp1->getOperand(1), m_Constant(C1)) ||
    ...
    PassPrediction::PassPeeper(42); // if
    return false;
  }
...
bool InstCombiner::SimplifyAssociativeOrCommutative(BinaryOperator &I) {
  ...
  do {
    ...
    // left (most complex).  This puts constants before unary operators before
    // binary operators.
    PassPrediction::PassPeeper(43); // do-while
    if (I.isCommutative() &&
        getComplexity(I.getOperand(0)) < getComplexity(I.getOperand(1))) {
      PassPrediction::PassPeeper(44); // if
    ...
    if (I.isAssociative()) {
      ...
      PassPrediction::PassPeeper(45); // if
      if (Op0 && Op0->getOpcode() == Opcode) {
        PassPrediction::PassPeeper(46); // if
        ...
        if (Value *V = SimplifyBinOp(Opcode, B, C, SQ.getWithInstruction(&I))) {
          ...
          PassPrediction::PassPeeper(47); // if
          I.setOperand(0, A);
          I.setOperand(1, V);
          ...
          if (MaintainNoSignedWrap(I, B, C) &&
            ...
            // the operands to Op0.
            PassPrediction::PassPeeper(48); // if
            I.clearSubclassOptionalData();
            I.setHasNoSignedWrap(true);
          } else {
            PassPrediction::PassPeeper(49); // else
      ...
      if (Op1 && Op1->getOpcode() == Opcode) {
        PassPrediction::PassPeeper(50); // if
        ...
        if (Value *V = SimplifyBinOp(Opcode, A, B, SQ.getWithInstruction(&I))) {
          ...
          PassPrediction::PassPeeper(51); // if
          I.setOperand(0, V);
          I.setOperand(1, C);
          // Conservatively clear the optional flags, since they may not be
          // preserved by the reassociation.
    ...
    if (I.isAssociative() && I.isCommutative()) {
      PassPrediction::PassPeeper(52); // if
      if (simplifyAssocCastAssoc(&I)) {
        PassPrediction::PassPeeper(53); // if
        Changed = true;
        ++NumReassoc;
        continue;
      }
      ...
      if (Op0 && Op0->getOpcode() == Opcode) {
        PassPrediction::PassPeeper(54); // if
        Value *A = Op0->getOperand(0);
        Value *B = Op0->getOperand(1);
        Value *C = I.getOperand(1);
        ...
        if (Value *V = SimplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
          ...
          PassPrediction::PassPeeper(55); // if
          I.setOperand(0, V);
          I.setOperand(1, B);
          // Conservatively clear the optional flags, since they may not be
          // preserved by the reassociation.
      ...
      if (Op1 && Op1->getOpcode() == Opcode) {
        PassPrediction::PassPeeper(56); // if
        Value *A = I.getOperand(0);
        Value *B = Op1->getOperand(0);
        Value *C = Op1->getOperand(1);
        ...
        if (Value *V = SimplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
          ...
          PassPrediction::PassPeeper(57); // if
          I.setOperand(0, B);
          I.setOperand(1, V);
          // Conservatively clear the optional flags, since they may not be
          // preserved by the reassociation.
      ...
      if (Op0 && Op1 && Op0->getOpcode() == Opcode &&
          ...
          Op1->hasOneUse()) {
        PassPrediction::PassPeeper(58); // if
        Value *A = Op0->getOperand(0);
        ...
        if (isa<FPMathOperator>(New)) {
          PassPrediction::PassPeeper(59); // if
...
static bool LeftDistributesOverRight(Instruction::BinaryOps LOp,
  ...
  case Instruction::And:
    PassPrediction::PassPeeper(60); // case
    ...
    default:
      return false;
    case Instruction::Or:
      PassPrediction::PassPeeper(61); // case

    case Instruction::Xor:
      PassPrediction::PassPeeper(62); // case

      return true;
    }

  case Instruction::Mul:
    PassPrediction::PassPeeper(63); // case
    ...
    case Instruction::Add:
      PassPrediction::PassPeeper(64); // case

    case Instruction::Sub:
      PassPrediction::PassPeeper(65); // case

      return true;
    }

  case Instruction::Or:
    PassPrediction::PassPeeper(66); // case
    ...
    default:
      return false;
    case Instruction::And:
      PassPrediction::PassPeeper(67); // case
...
static bool RightDistributesOverLeft(Instruction::BinaryOps LOp,
  ...
  if (Instruction::isCommutative(ROp)) {
    PassPrediction::PassPeeper(68); // if
  ...
  // (X >> Z) ^ (Y >> Z)  -> (X^Y) >> Z  for all shifts.
  case Instruction::And:
    PassPrediction::PassPeeper(69); // case

  case Instruction::Or:
    PassPrediction::PassPeeper(70); // case

  case Instruction::Xor:
    PassPrediction::PassPeeper(71); // case

    switch (ROp) {
    default:
      return false;
    case Instruction::Shl:
      PassPrediction::PassPeeper(72); // case

    case Instruction::LShr:
      PassPrediction::PassPeeper(73); // case

    case Instruction::AShr:
      PassPrediction::PassPeeper(74); // case
...
static Value *getIdentityValue(Instruction::BinaryOps Opcode, Value *V) {
  if (isa<Constant>(V)) {
    PassPrediction::PassPeeper(75); // if
    return nullptr;
  }
...
getBinOpsForFactorization(Instruction::BinaryOps TopLevelOpcode,
  ...
  case Instruction::Add:
    PassPrediction::PassPeeper(76); // case

  case Instruction::Sub:
    PassPrediction::PassPeeper(77); // case

    if (Op->getOpcode() == Instruction::Shl) {
      PassPrediction::PassPeeper(78); // if
      if (Constant *CST = dyn_cast<Constant>(Op->getOperand(1))) {
        // The multiplier is really 1 << CST.
        PassPrediction::PassPeeper(79); // if
...
Value *InstCombiner::tryFactorization(BinaryOperator &I,
  ...
  if (LeftDistributesOverRight(InnerOpcode, TopLevelOpcode)) {
    ...
    PassPrediction::PassPeeper(80); // if
    if (A == C || (InnerCommutative && A == D)) {
      PassPrediction::PassPeeper(81); // if
      if (A != C) {
        PassPrediction::PassPeeper(82); // if
        std::swap(C, D);
      }
      ...
      if (!V && LHS->hasOneUse() && RHS->hasOneUse()) {
        PassPrediction::PassPeeper(83); // if
        V = Builder.CreateBinOp(TopLevelOpcode, B, D, RHS->getName());
      }
      if (V) {
        PassPrediction::PassPeeper(84); // if
  ...
  if (!SimplifiedInst &&
    ...
    PassPrediction::PassPeeper(85); // if
    if (B == D || (InnerCommutative && B == C)) {
      PassPrediction::PassPeeper(86); // if
      if (B != D) {
        PassPrediction::PassPeeper(87); // if
        std::swap(C, D);
      }
      ...
      // operations "A op' B" and "C op' D" will be zapped as no longer used.
      if (!V && LHS->hasOneUse() && RHS->hasOneUse()) {
        PassPrediction::PassPeeper(88); // if
        V = Builder.CreateBinOp(TopLevelOpcode, A, C, LHS->getName());
      }
      if (V) {
        PassPrediction::PassPeeper(89); // if
  ...
  if (SimplifiedInst) {
    PassPrediction::PassPeeper(90); // if
    ++NumFactor;
    ...
    if (BinaryOperator *BO = dyn_cast<BinaryOperator>(SimplifiedInst)) {
      PassPrediction::PassPeeper(91); // if
      if (isa<OverflowingBinaryOperator>(SimplifiedInst)) {
        PassPrediction::PassPeeper(92); // if
        bool HasNSW = false;
        if (isa<OverflowingBinaryOperator>(&I)) {
          PassPrediction::PassPeeper(93); // if
          HasNSW = I.hasNoSignedWrap();
        }

        if (auto *LOBO = dyn_cast<OverflowingBinaryOperator>(LHS)) {
          PassPrediction::PassPeeper(94); // if
          HasNSW &= LOBO->hasNoSignedWrap();
        }

        if (auto *ROBO = dyn_cast<OverflowingBinaryOperator>(RHS)) {
          PassPrediction::PassPeeper(95); // if
        ...
        if (TopLevelOpcode == Instruction::Add &&
          ...
          PassPrediction::PassPeeper(96); // if
          if (match(V, m_APInt(CInt)) && !CInt->isMinSignedValue()) {
            PassPrediction::PassPeeper(97); // if
...
Value *InstCombiner::SimplifyUsingDistributiveLaws(BinaryOperator &I) {
...

  {
    ...
    if (Op0) {
      PassPrediction::PassPeeper(98); // if
      LHSOpcode = getBinOpsForFactorization(TopLevelOpcode, Op0, A, B);
    }
    if (Op1) {
      PassPrediction::PassPeeper(99); // if
    ...
    if (Op0 && Op1 && LHSOpcode == RHSOpcode) {
      PassPrediction::PassPeeper(100); // if
      if (Value *V = tryFactorization(I, LHSOpcode, A, B, C, D)) {
        PassPrediction::PassPeeper(101); // if
        return V;
      }
    }
    ...
    // term.
    if (Op0) {
      PassPrediction::PassPeeper(102); // if
      if (Value *Ident = getIdentityValue(LHSOpcode, RHS)) {
        PassPrediction::PassPeeper(103); // if
        if (Value *V = tryFactorization(I, LHSOpcode, A, B, RHS, Ident)) {
          PassPrediction::PassPeeper(104); // if
          return V;
        }
      }
    }
    ...
    // term.
    if (Op1) {
      PassPrediction::PassPeeper(105); // if
      if (Value *Ident = getIdentityValue(RHSOpcode, LHS)) {
        PassPrediction::PassPeeper(106); // if
        if (Value *V = tryFactorization(I, RHSOpcode, LHS, Ident, C, D)) {
          PassPrediction::PassPeeper(107); // if
          return V;
        }
      }
    }
  ...
  if (Op0 && RightDistributesOverLeft(Op0->getOpcode(), TopLevelOpcode)) {
    ...
    PassPrediction::PassPeeper(108); // if
    ...
    if (L && R) {
      ...
      PassPrediction::PassPeeper(109); // if
      ++NumExpand;
    ...
    if (L && L == ConstantExpr::getBinOpIdentity(InnerOpcode, L->getType())) {
      ...
      PassPrediction::PassPeeper(110); // if
      ++NumExpand;
    ...
    if (R && R == ConstantExpr::getBinOpIdentity(InnerOpcode, R->getType())) {
      ...
      PassPrediction::PassPeeper(111); // if
      ++NumExpand;
  ...
  if (Op1 && LeftDistributesOverRight(TopLevelOpcode, Op1->getOpcode())) {
    ...
    PassPrediction::PassPeeper(112); // if
    ...
    if (L && R) {
      // They do! Return "L op' R".
      PassPrediction::PassPeeper(113); // if
      ++NumExpand;
    ...
    if (L && L == ConstantExpr::getBinOpIdentity(InnerOpcode, L->getType())) {
      // They do! Return "A op C".
      PassPrediction::PassPeeper(114); // if
      ++NumExpand;
    ...
    if (R && R == ConstantExpr::getBinOpIdentity(InnerOpcode, R->getType())) {
      ...
      PassPrediction::PassPeeper(115); // if
      ++NumExpand;
  ...
  if (auto *SI0 = dyn_cast<SelectInst>(LHS)) {
    PassPrediction::PassPeeper(116); // if
    if (auto *SI1 = dyn_cast<SelectInst>(RHS)) {
      PassPrediction::PassPeeper(117); // if
      if (SI0->getCondition() == SI1->getCondition()) {
        PassPrediction::PassPeeper(118); // if
        Value *SI = nullptr;
        if (Value *V = SimplifyBinOp(TopLevelOpcode, SI0->getFalseValue(),
                                     SI1->getFalseValue(),
                                     SQ.getWithInstruction(&I))) {
          PassPrediction::PassPeeper(119); // if
        ...
        if (Value *V =
          ...
          PassPrediction::PassPeeper(120); // if
        ...
        if (SI) {
          PassPrediction::PassPeeper(121); // if
          SI->takeName(&I);
          return SI;
        }
      }
...
Value *InstCombiner::dyn_castNegVal(Value *V) const {
  if (BinaryOperator::isNeg(V)) {
    PassPrediction::PassPeeper(122); // if
    return BinaryOperator::getNegArgument(V);
  }

  // Constants can be considered to be negated values if they can be folded.
  if (ConstantInt *C = dyn_cast<ConstantInt>(V)) {
    PassPrediction::PassPeeper(123); // if
    return ConstantExpr::getNeg(C);
  }

  if (ConstantDataVector *C = dyn_cast<ConstantDataVector>(V)) {
    PassPrediction::PassPeeper(124); // if
    if (C->getType()->getElementType()->isIntegerTy()) {
      PassPrediction::PassPeeper(125); // if
      return ConstantExpr::getNeg(C);
    }
  }

  if (ConstantVector *CV = dyn_cast<ConstantVector>(V)) {
    PassPrediction::PassPeeper(126); // if
    for (unsigned i = 0, e = CV->getNumOperands(); i != e; ++i) {
      PassPrediction::PassPeeper(127); // for
      Constant *Elt = CV->getAggregateElement(i);
      if (!Elt) {
        PassPrediction::PassPeeper(128); // if
        return nullptr;
      }

      if (isa<UndefValue>(Elt)) {
        PassPrediction::PassPeeper(129); // if
        continue;
      }

      if (!isa<ConstantInt>(Elt)) {
        PassPrediction::PassPeeper(130); // if
        return nullptr;
      }
    }
...
Value *InstCombiner::dyn_castFNegVal(Value *V, bool IgnoreZeroSign) const {
  if (BinaryOperator::isFNeg(V, IgnoreZeroSign)) {
    PassPrediction::PassPeeper(131); // if
    return BinaryOperator::getFNegArgument(V);
  }

  // Constants can be considered to be negated values if they can be folded.
  if (ConstantFP *C = dyn_cast<ConstantFP>(V)) {
    PassPrediction::PassPeeper(132); // if
    return ConstantExpr::getFNeg(C);
  }

  if (ConstantDataVector *C = dyn_cast<ConstantDataVector>(V)) {
    PassPrediction::PassPeeper(133); // if
    if (C->getType()->getElementType()->isFloatingPointTy()) {
      PassPrediction::PassPeeper(134); // if
      return ConstantExpr::getFNeg(C);
    }
  }
...
static Value *foldOperationIntoSelectOperand(Instruction &I, Value *SO,
  ...
  if (auto *Cast = dyn_cast<CastInst>(&I)) {
    PassPrediction::PassPeeper(135); // if
  ...
  if (auto *SOC = dyn_cast<Constant>(SO)) {
    PassPrediction::PassPeeper(136); // if
    if (ConstIsRHS) {
      PassPrediction::PassPeeper(137); // if
  ...
  if (!ConstIsRHS) {
    PassPrediction::PassPeeper(138); // if
    std::swap(Op0, Op1);
  }
  ...
  if (FPInst && isa<FPMathOperator>(FPInst)) {
    PassPrediction::PassPeeper(139); // if
...
Instruction *InstCombiner::FoldOpIntoSelect(Instruction &Op, SelectInst *SI) {
  ...
  if (!SI->hasOneUse()) {
    PassPrediction::PassPeeper(140); // if
    return nullptr;
  }
  ...
  if (!(isa<Constant>(TV) || isa<Constant>(FV))) {
    PassPrediction::PassPeeper(141); // if
    return nullptr;
  }

  // Bool selects with constant operands can be folded to logical ops.
  if (SI->getType()->isIntOrIntVectorTy(1)) {
    PassPrediction::PassPeeper(142); // if
    return nullptr;
  }
  ...
  if (auto *BC = dyn_cast<BitCastInst>(&Op)) {
    PassPrediction::PassPeeper(143); // if
    VectorType *DestTy = dyn_cast<VectorType>(BC->getDestTy());
    VectorType *SrcTy = dyn_cast<VectorType>(BC->getSrcTy());

    // Verify that either both or neither are vectors.
    if ((SrcTy == nullptr) != (DestTy == nullptr)) {
      PassPrediction::PassPeeper(144); // if
      return nullptr;
    }

    // If vectors, verify that they have the same number of elements.
    if (SrcTy && SrcTy->getNumElements() != DestTy->getNumElements()) {
      PassPrediction::PassPeeper(145); // if
      return nullptr;
    }
  }
  ...
  if (auto *CI = dyn_cast<CmpInst>(SI->getCondition())) {
    PassPrediction::PassPeeper(146); // if
    if (CI->hasOneUse()) {
      PassPrediction::PassPeeper(147); // if
      Value *Op0 = CI->getOperand(0), *Op1 = CI->getOperand(1);
      if ((SI->getOperand(1) == Op0 && SI->getOperand(2) == Op1) ||
          (SI->getOperand(2) == Op0 && SI->getOperand(1) == Op1)) {
        PassPrediction::PassPeeper(148); // if
        return nullptr;
      }
    }
  }
...
static Value *foldOperationIntoPhiValue(BinaryOperator *I, Value *InV,
  ...
  if (auto *InC = dyn_cast<Constant>(InV)) {
    PassPrediction::PassPeeper(149); // if
    if (ConstIsRHS) {
      PassPrediction::PassPeeper(150); // if
  ...
  Value *Op0 = InV, *Op1 = C;
  if (!ConstIsRHS) {
    PassPrediction::PassPeeper(151); // if
    std::swap(Op0, Op1);
  }
  ...
  auto *FPInst = dyn_cast<Instruction>(RI);
  if (FPInst && isa<FPMathOperator>(FPInst)) {
    PassPrediction::PassPeeper(152); // if
...
Instruction *InstCombiner::foldOpIntoPhi(Instruction &I, PHINode *PN) {
  ...
  if (NumPHIValues == 0) {
    PassPrediction::PassPeeper(153); // if
    return nullptr;
  }
  ...
  if (!PN->hasOneUse()) {
    ...
    PassPrediction::PassPeeper(154); // if
    for (User *U : PN->users()) {
      PassPrediction::PassPeeper(155); // for-range
      Instruction *UI = cast<Instruction>(U);
      if (UI != &I && !I.isIdenticalTo(UI)) {
        PassPrediction::PassPeeper(156); // if
        return nullptr;
      }
    }
  ...
  for (unsigned i = 0; i != NumPHIValues; ++i) {
    PassPrediction::PassPeeper(157); // for
    Value *InVal = PN->getIncomingValue(i);
    if (isa<Constant>(InVal) && !isa<ConstantExpr>(InVal)) {
      PassPrediction::PassPeeper(158); // if
      continue;
    }

    if (isa<PHINode>(InVal)) {
      PassPrediction::PassPeeper(159); // if
      return nullptr;                  // Itself a phi.
    }
    if (NonConstBB) {
      PassPrediction::PassPeeper(160); // if
    ...
    if (InvokeInst *II = dyn_cast<InvokeInst>(InVal)) {
      PassPrediction::PassPeeper(161); // if
      if (II->getParent() == NonConstBB) {
        PassPrediction::PassPeeper(162); // if
        return nullptr;
      }
    }
    ...
    if (isPotentiallyReachable(I.getParent(), NonConstBB, &DT, LI)) {
      PassPrediction::PassPeeper(163); // if
      return nullptr;
    }
  }
  ...
  if (NonConstBB != nullptr) {
    PassPrediction::PassPeeper(164); // if
    BranchInst *BI = dyn_cast<BranchInst>(NonConstBB->getTerminator());
    if (!BI || !BI->isUnconditional()) {
      PassPrediction::PassPeeper(165); // if
      return nullptr;
    }
  }
  ...
  if (NonConstBB) {
    PassPrediction::PassPeeper(166); // if
  ...
  if (SelectInst *SI = dyn_cast<SelectInst>(&I)) {
    ...
    PassPrediction::PassPeeper(167); // if
    Value *TrueV = SI->getTrueValue();
    Value *FalseV = SI->getFalseValue();
    BasicBlock *PhiTransBB = PN->getParent();
    for (unsigned i = 0; i != NumPHIValues; ++i) {
      PassPrediction::PassPeeper(168); // for
      ...
      if (InC && !isa<ConstantExpr>(InC) && isa<ConstantInt>(InC)) {
        PassPrediction::PassPeeper(169); // if
        ...
        PassPrediction::PassPeeper(170); // else
    ...
    PassPrediction::PassPeeper(171); // if
    Constant *C = cast<Constant>(I.getOperand(1));
    for (unsigned i = 0; i != NumPHIValues; ++i) {
      PassPrediction::PassPeeper(172); // for
      Value *InV = nullptr;
      if (Constant *InC = dyn_cast<Constant>(PN->getIncomingValue(i))) {
        PassPrediction::PassPeeper(173); // if
        InV = ConstantExpr::getCompare(CI->getPredicate(), InC, C);
      } else if (isa<ICmpInst>(CI)) {
        PassPrediction::PassPeeper(174); // if
        InV = Builder.CreateICmp(CI->getPredicate(), PN->getIncomingValue(i), C,
                                 "phitmp");
      } else {
        PassPrediction::PassPeeper(175); // else
    ...
    PassPrediction::PassPeeper(176); // if
    for (unsigned i = 0; i != NumPHIValues; ++i) {
      PassPrediction::PassPeeper(178); // for
      Value *InV =
          foldOperationIntoPhiValue(BO, PN->getIncomingValue(i), Builder);
      NewPN->addIncoming(InV, PN->getIncomingBlock(i));
    }
  } else {
    PassPrediction::PassPeeper(177); // else
    CastInst *CI = cast<CastInst>(&I);
    Type *RetTy = CI->getType();
    for (unsigned i = 0; i != NumPHIValues; ++i) {
      PassPrediction::PassPeeper(179); // for
      Value *InV;
      if (Constant *InC = dyn_cast<Constant>(PN->getIncomingValue(i))) {
        PassPrediction::PassPeeper(180); // if
        InV = ConstantExpr::getCast(CI->getOpcode(), InC, RetTy);
      } else {
        PassPrediction::PassPeeper(181); // else
  ...
  for (auto UI = PN->user_begin(), E = PN->user_end(); UI != E;) {
    PassPrediction::PassPeeper(182); // for
    Instruction *User = cast<Instruction>(*UI++);
    if (User == &I) {
      PassPrediction::PassPeeper(183); // if
      continue;
    }
...
Instruction *InstCombiner::foldOpWithConstantIntoOperand(BinaryOperator &I) {
  ...
  if (auto *Sel = dyn_cast<SelectInst>(I.getOperand(0))) {
    PassPrediction::PassPeeper(184); // if
    if (Instruction *NewSel = FoldOpIntoSelect(I, Sel)) {
      PassPrediction::PassPeeper(185); // if
      return NewSel;
    }
  } else if (auto *PN = dyn_cast<PHINode>(I.getOperand(0))) {
    PassPrediction::PassPeeper(186); // if
    if (Instruction *NewPhi = foldOpIntoPhi(I, PN)) {
      PassPrediction::PassPeeper(187); // if
      return NewPhi;
    }
  }
  return nullptr;
...
Type *InstCombiner::FindElementAtOffset(PointerType *PtrTy, int64_t Offset,
  ...
  if (!Ty->isSized()) {
    PassPrediction::PassPeeper(188); // if
    return nullptr;
  }
  ...
  if (int64_t TySize = DL.getTypeAllocSize(Ty)) {
    PassPrediction::PassPeeper(189); // if
    FirstIdx = Offset / TySize;
    Offset -= FirstIdx * TySize;

    // Handle hosts where % returns negative instead of values [0..TySize).
    if (Offset < 0) {
      PassPrediction::PassPeeper(190); // if
      --FirstIdx;
      Offset += TySize;
      assert(Offset >= 0);
    }
  ...
  while (Offset) {
    ...
    PassPrediction::PassPeeper(191); // while
    if (uint64_t(Offset * 8) >= DL.getTypeSizeInBits(Ty)) {
      PassPrediction::PassPeeper(192); // if
      return nullptr;
    }

    if (StructType *STy = dyn_cast<StructType>(Ty)) {
      PassPrediction::PassPeeper(193); // if
      ...
      PassPrediction::PassPeeper(194); // if
      ...
      PassPrediction::PassPeeper(195); // else
      return nullptr;
    }
  }
...
static bool shouldMergeGEPs(GEPOperator &GEP, GEPOperator &Src) {
  ...
  if (GEP.hasAllZeroIndices() && !Src.hasAllZeroIndices() && !Src.hasOneUse()) {
    PassPrediction::PassPeeper(196); // if
    return false;
  }
  return true;
}
...
Value *InstCombiner::Descale(Value *Val, APInt Scale, bool &NoSignedWrap) {
  ...
  if (match(Val, m_Zero()) || Scale == 1) {
    PassPrediction::PassPeeper(197); // if
    NoSignedWrap = true;
    return Val;
  }
  ...
  if (Scale.isMinValue()) {
    PassPrediction::PassPeeper(198); // if
    return nullptr;
  }
  ...
  for (;; Op = Parent.first->getOperand(Parent.second)) { // Drill down
    ...
    PassPrediction::PassPeeper(199); // for
    if (ConstantInt *CI = dyn_cast<ConstantInt>(Op)) {
      // If Op is a constant divisible by Scale then descale to the quotient.
      PassPrediction::PassPeeper(200);         // if
      APInt Quotient(Scale), Remainder(Scale); // Init ensures right bitwidth.
      APInt::sdivrem(CI->getValue(), Scale, Quotient, Remainder);
      if (!Remainder.isMinValue()) {
        // Not divisible by Scale.
        PassPrediction::PassPeeper(201); // if
        return nullptr;
      }
      // Replace with the quotient in the parent.
      Op = ConstantInt::get(CI->getType(), Quotient);
      NoSignedWrap = true;
      PassPrediction::PassPeeper(202); // break
      break;
    }

    if (BinaryOperator *BO = dyn_cast<BinaryOperator>(Op)) {

      PassPrediction::PassPeeper(203); // if
      if (BO->getOpcode() == Instruction::Mul) {
        // Multiplication.
        PassPrediction::PassPeeper(204); // if
        NoSignedWrap = BO->hasNoSignedWrap();
        if (RequireNoSignedWrap && !NoSignedWrap) {
          PassPrediction::PassPeeper(205); // if
          return nullptr;
        }
        ...
        if (ConstantInt *CI = dyn_cast<ConstantInt>(RHS)) {
          ...
          PassPrediction::PassPeeper(206); // if
          if (CI->getValue() == Scale) {
            // Multiplication by exactly the scale, replace the multiplication
            // by its left-hand side in the parent.
            PassPrediction::PassPeeper(207); // if
            Op = LHS;
            PassPrediction::PassPeeper(208); // break
            break;
          }

          // Otherwise drill down into the constant.
          if (!Op->hasOneUse()) {
            PassPrediction::PassPeeper(209); // if
            return nullptr;
          }
        ...
        if (!Op->hasOneUse()) {
          PassPrediction::PassPeeper(210); // if
          return nullptr;
        }
      ...
      if (logScale > 0 && BO->getOpcode() == Instruction::Shl &&
        ...
        PassPrediction::PassPeeper(211); // if
        NoSignedWrap = BO->hasNoSignedWrap();
        if (RequireNoSignedWrap && !NoSignedWrap) {
          PassPrediction::PassPeeper(212); // if
          return nullptr;
        }
        ...
        if (Amt == logScale) {
          // Multiplication by exactly the scale, replace the multiplication
          // by its left-hand side in the parent.
          PassPrediction::PassPeeper(213); // if
          Op = LHS;
          PassPrediction::PassPeeper(214); // break
          break;
        }
        if (Amt < logScale || !Op->hasOneUse()) {
          PassPrediction::PassPeeper(215); // if
          return nullptr;
        }
        ...
        PassPrediction::PassPeeper(216); // break
        break;
      }
    }

    if (!Op->hasOneUse()) {
      PassPrediction::PassPeeper(217); // if
      return nullptr;
    }

    if (CastInst *Cast = dyn_cast<CastInst>(Op)) {
      PassPrediction::PassPeeper(218); // if
      if (Cast->getOpcode() == Instruction::SExt) {
        // Op is sign-extended from a smaller type, descale in the smaller type.
        PassPrediction::PassPeeper(219); // if
        ...
        if (SmallScale.sext(Scale.getBitWidth()) != Scale) {
          ...
          PassPrediction::PassPeeper(220); // if
          return nullptr;
        }
      ...
      if (Cast->getOpcode() == Instruction::Trunc) {
        ...
        PassPrediction::PassPeeper(221); // if
        if (RequireNoSignedWrap) {
          PassPrediction::PassPeeper(222); // if
          return nullptr;
        }
        ...
        if (logScale + 1 ==
          ...
          PassPrediction::PassPeeper(223); // if
          logScale = -1;
        }
  ...
  if (match(Op, m_Zero())) {
    PassPrediction::PassPeeper(224); // if
    NoSignedWrap = true;
    return Op;
  }
  ...
  if (!Parent.first) {
    ...
    PassPrediction::PassPeeper(225); // if
    return Op;
  }
  ...
  do {
    PassPrediction::PassPeeper(226); // do-while
    if (BinaryOperator *BO = dyn_cast<BinaryOperator>(Ancestor)) {
      ...
      PassPrediction::PassPeeper(227); // if
      bool OpNoSignedWrap = BO->hasNoSignedWrap();
      NoSignedWrap &= OpNoSignedWrap;
      if (NoSignedWrap != OpNoSignedWrap) {
        PassPrediction::PassPeeper(228); // if
      ...
      PassPrediction::PassPeeper(229); // if
      NoSignedWrap = false;
    }
    ...
    if (Ancestor == Val) {
      ...
      PassPrediction::PassPeeper(230); // if
      return Val;
    }
...
static Value *CreateBinOpAsGiven(BinaryOperator &Inst, Value *LHS, Value *RHS,
  ...
  if (BinaryOperator *NewBO = dyn_cast<BinaryOperator>(BO)) {
    PassPrediction::PassPeeper(231); // if
...
Value *InstCombiner::SimplifyVectorOp(BinaryOperator &Inst) {
  if (!Inst.getType()->isVectorTy()) {
    PassPrediction::PassPeeper(232); // if
    return nullptr;
  }
  ...
  if (!isSafeToSpeculativelyExecute(&Inst)) {
    PassPrediction::PassPeeper(233); // if
    return nullptr;
  }
  ...
  if (LShuf && RShuf && LShuf->getMask() == RShuf->getMask() &&
    ...
    PassPrediction::PassPeeper(234); // if
  ...
  if (isa<ShuffleVectorInst>(LHS)) {
    PassPrediction::PassPeeper(235); // if
    Shuffle = cast<ShuffleVectorInst>(LHS);
  }
  if (isa<ShuffleVectorInst>(RHS)) {
    PassPrediction::PassPeeper(236); // if
    Shuffle = cast<ShuffleVectorInst>(RHS);
  }
  if (isa<Constant>(LHS)) {
    PassPrediction::PassPeeper(237); // if
    C1 = cast<Constant>(LHS);
  }
  if (isa<Constant>(RHS)) {
    PassPrediction::PassPeeper(238); // if
  ...
  if (Shuffle && C1 &&
    ...
    PassPrediction::PassPeeper(239); // if
    ...
    for (unsigned I = 0; I < VWidth; ++I) {
      PassPrediction::PassPeeper(240); // for
      if (ShMask[I] >= 0) {
        assert(ShMask[I] < (int)VWidth);
        if (!isa<UndefValue>(C2M[ShMask[I]])) {
          PassPrediction::PassPeeper(241); // if
          MayChange = false;
          PassPrediction::PassPeeper(242); // break
          break;
        }
    ...
    if (MayChange) {
      PassPrediction::PassPeeper(243); // if
...
Instruction *InstCombiner::visitGetElementPtrInst(GetElementPtrInst &GEP) {
  ...
  if (Value *V = SimplifyGEPInst(GEP.getSourceElementType(), Ops,
    ...
    PassPrediction::PassPeeper(244); // if
  ...
  for (User::op_iterator I = GEP.op_begin() + 1, E = GEP.op_end(); I != E;
    ...
    PassPrediction::PassPeeper(245); // for
    if (GTI.isStruct()) {
      PassPrediction::PassPeeper(246); // if
      continue;
    }
    ...
    if (EltTy->isSized() && DL.getTypeAllocSize(EltTy) == 0) {
      PassPrediction::PassPeeper(247); // if
      if (!isa<Constant>(*I) || !cast<Constant>(*I)->isNullValue()) {
        PassPrediction::PassPeeper(248); // if
  ...
  if (PHINode *PN = dyn_cast<PHINode>(PtrOp)) {
    ...
    // loop iteration).
    if (Op1 == &GEP) {
      PassPrediction::PassPeeper(253); // if
      return nullptr;
    }
    ...
    for (auto I = PN->op_begin() + 1, E = PN->op_end(); I != E; ++I) {
      ...
      for (unsigned J = 0, F = Op1->getNumOperands(); J != F; ++J) {
        PassPrediction::PassPeeper(257); // for
        if (Op1->getOperand(J)->getType() != Op2->getOperand(J)->getType()) {
          PassPrediction::PassPeeper(258); // if
          return nullptr;
        }

        if (Op1->getOperand(J) != Op2->getOperand(J)) {
          PassPrediction::PassPeeper(259); // if
          if (DI == -1) {
            ...
            PassPrediction::PassPeeper(260); // if
            if (J > 1 && CurTy->isStructTy()) {
              PassPrediction::PassPeeper(262); // if
              return nullptr;
            }
            ...
            PassPrediction::PassPeeper(261); // else
            return nullptr;
          }
        }
        ...
        if (J > 0) {
          PassPrediction::PassPeeper(263); // if
          if (J == 1) {
            PassPrediction::PassPeeper(264); // if
            CurTy = Op1->getSourceElementType();
          } else if (CompositeType *CT = dyn_cast<CompositeType>(CurTy)) {
            PassPrediction::PassPeeper(265); // if
            CurTy = CT->getTypeAtIndex(Op1->getOperand(J));
          } else {
            PassPrediction::PassPeeper(266); // else
            CurTy = nullptr;
          }
        }
      }
    ...
    if (DI != -1 && !PN->hasOneUse()) {
      PassPrediction::PassPeeper(267); // if
      return nullptr;
    }
    ...
    if (DI == -1) {
      ...
      PassPrediction::PassPeeper(268); // if
      ...
      // set that index.
      PassPrediction::PassPeeper(269); // else
      PHINode *NewPN;
      {
      ...
      for (auto &I : PN->operands()) {
        PassPrediction::PassPeeper(270); // for-range
  ...
  if (GEPOperator *Src = dyn_cast<GEPOperator>(PtrOp)) {
    PassPrediction::PassPeeper(271); // if
    if (!shouldMergeGEPs(*cast<GEPOperator>(&GEP), *Src)) {
      PassPrediction::PassPeeper(272); // if
      return nullptr;
    }
    ...
    if (GEPOperator *SrcGEP = dyn_cast<GEPOperator>(Src->getOperand(0))) {
      PassPrediction::PassPeeper(273); // if
      if (SrcGEP->getNumOperands() == 2 && shouldMergeGEPs(*Src, *SrcGEP)) {
        PassPrediction::PassPeeper(274); // if
    ...
    for (gep_type_iterator I = gep_type_begin(*Src), E = gep_type_end(*Src);
         I != E; ++I) {
      PassPrediction::PassPeeper(275); // for
    ...
    if (EndsWithSequential) {
      ...
      //
      PassPrediction::PassPeeper(276); // if
      ...
      if (SO1->getType() != GO1->getType()) {
        PassPrediction::PassPeeper(277); // if
        return nullptr;
      }
      ...
      if (Sum == nullptr) {
        PassPrediction::PassPeeper(278); // if
        return nullptr;
      }

      // Update the GEP in place if possible.
      if (Src->getNumOperands() == 2) {
        PassPrediction::PassPeeper(279); // if
      ...
      PassPrediction::PassPeeper(280); // if
      Indices.append(Src->op_begin() + 1, Src->op_end());
      Indices.append(GEP.idx_begin() + 1, GEP.idx_end());
    }

    if (!Indices.empty()) {
      PassPrediction::PassPeeper(281); // if
  ...
  if (GEP.getNumIndices() == 1) {
    PassPrediction::PassPeeper(282); // if
    unsigned AS = GEP.getPointerAddressSpace();
    if (GEP.getOperand(1)->getType()->getScalarSizeInBits() ==
        DL.getPointerSizeInBits(AS)) {
      PassPrediction::PassPeeper(283); // if
      ...
      Value *V = nullptr;
      if (TyAllocSize == 1) {
        PassPrediction::PassPeeper(284); // if
        V = GEP.getOperand(1);
        Matched = true;
      } else if (match(GEP.getOperand(1),
                       m_AShr(m_Value(V), m_ConstantInt(C)))) {
        PassPrediction::PassPeeper(285); // if
        if (TyAllocSize == 1ULL << C) {
          PassPrediction::PassPeeper(286); // if
          Matched = true;
        }
      } else if (match(GEP.getOperand(1),
                       m_SDiv(m_Value(V), m_ConstantInt(C)))) {
        PassPrediction::PassPeeper(287); // if
        if (TyAllocSize == C) {
          PassPrediction::PassPeeper(288); // if
          Matched = true;
        }
      }
      ...
      if (Matched) {
        ...
        // pointer arithmetic.
        PassPrediction::PassPeeper(289); // if
        if (match(V, m_Neg(m_PtrToInt(m_Value())))) {
          PassPrediction::PassPeeper(290); // if
        ...
        if (match(V, m_Sub(m_PtrToInt(m_Value(Y)),
          ...
          PassPrediction::PassPeeper(291); // if
  ...
  if (GEP.getType()->isVectorTy()) {
    PassPrediction::PassPeeper(292); // if
    return nullptr;
  }
  ...
  if (StrippedPtr != PtrOp) {
    PassPrediction::PassPeeper(293); // if
    bool HasZeroPointerIndex = false;
    if (ConstantInt *C = dyn_cast<ConstantInt>(GEP.getOperand(1))) {
      PassPrediction::PassPeeper(294); // if
    ...
    if (HasZeroPointerIndex) {
      ...
      if (ArrayType *CATy = dyn_cast<ArrayType>(GEP.getSourceElementType())) {
        ...
        if (ArrayType *XATy =
          ...
          PassPrediction::PassPeeper(299); // if
          if (CATy->getElementType() == XATy->getElementType()) {
            ...
            PassPrediction::PassPeeper(300); // if
            if (StrippedPtrTy->getAddressSpace() == GEP.getAddressSpace()) {
              PassPrediction::PassPeeper(301); // if
      ...
      if (ResElTy->isSized() && SrcElTy->isSized()) {
        ...
        if (ResSize && SrcSize % ResSize == 0) {
          ...
          if (Value *NewIdx = Descale(Idx, APInt(BitWidth, Scale), NSW)) {
            ...
            PassPrediction::PassPeeper(306); // if
            Value *NewGEP =
                GEP.isInBounds() && NSW
      ...
      if (ResElTy->isSized() && SrcElTy->isSized() && SrcElTy->isArrayTy()) {
        ...
        PassPrediction::PassPeeper(307); // if
        uint64_t ResSize = DL.getTypeAllocSize(ResElTy);
        uint64_t ArrayEltSize =
            DL.getTypeAllocSize(SrcElTy->getArrayElementType());
        if (ResSize && ArrayEltSize % ResSize == 0) {
          PassPrediction::PassPeeper(308); // if
          Value *Idx = GEP.getOperand(1);
          unsigned BitWidth = Idx->getType()->getPrimitiveSizeInBits();
          ...
          if (Value *NewIdx = Descale(Idx, APInt(BitWidth, Scale), NSW)) {
            ...
            // If the multiplication NewIdx * Scale may overflow then the new
            // GEP may not be "inbounds".
            PassPrediction::PassPeeper(309); // if
            Value *Off[2] = {
  ...
  if (AddrSpaceCastInst *ASC = dyn_cast<AddrSpaceCastInst>(PtrOp)) {
    ...
    PassPrediction::PassPeeper(310); // if
    if (BitCastInst *BC = dyn_cast<BitCastInst>(ASC->getOperand(0))) {
      PassPrediction::PassPeeper(311); // if
      PtrOp = BC;
    }
  }
  ...
  if (BitCastInst *BCI = dyn_cast<BitCastInst>(PtrOp)) {
    PassPrediction::PassPeeper(312); // if
    ...
    if (!isa<BitCastInst>(Operand) &&
      ...
      PassPrediction::PassPeeper(313); // if
      if (!Offset) {
        // If the bitcast is of an allocation, and the allocation will be
        // converted to match the type of the cast, don't touch this.
        PassPrediction::PassPeeper(314); // if
        if (isa<AllocaInst>(Operand) || isAllocationFn(Operand, &TLI)) {
          // See if the bitcast simplifies, if so, don't nuke this GEP yet.
          PassPrediction::PassPeeper(315); // if
          if (Instruction *I = visitBitCast(*BCI)) {
            PassPrediction::PassPeeper(316); // if
            if (I != BCI) {
              PassPrediction::PassPeeper(317); // if
              I->takeName(BCI);
        ...
        if (Operand->getType()->getPointerAddressSpace() !=
            GEP.getAddressSpace()) {
          PassPrediction::PassPeeper(318); // if
      ...
      if (FindElementAtOffset(OpType, Offset.getSExtValue(), NewIndices)) {
        PassPrediction::PassPeeper(319); // if
        Value *NGEP =
            GEP.isInBounds()
        ...
        if (NGEP->getType() == GEP.getType()) {
          PassPrediction::PassPeeper(320); // if
        ...
        if (NGEP->getType()->getPointerAddressSpace() !=
            GEP.getAddressSpace()) {
          PassPrediction::PassPeeper(321); // if
  ...
  if (!GEP.isInBounds()) {
    PassPrediction::PassPeeper(322); // if
    unsigned PtrWidth =
    ...
    if (auto *AI = dyn_cast<AllocaInst>(UnderlyingPtrOp)) {
      PassPrediction::PassPeeper(323); // if
      if (GEP.accumulateConstantOffset(DL, BasePtrOffset) &&
          BasePtrOffset.isNonNegative()) {
        PassPrediction::PassPeeper(324); // if
        APInt AllocSize(PtrWidth, DL.getTypeAllocSize(AI->getAllocatedType()));
        if (BasePtrOffset.ule(AllocSize)) {
          PassPrediction::PassPeeper(325); // if
...
static bool isNeverEqualToUnescapedAlloc(Value *V, const TargetLibraryInfo *TLI,
  ...
  if (isa<ConstantPointerNull>(V)) {
    PassPrediction::PassPeeper(326); // if
    return true;
  }
  if (auto *LI = dyn_cast<LoadInst>(V)) {
    PassPrediction::PassPeeper(327); // if
...
static bool isAllocSiteRemovable(Instruction *AI,
  ...
  do {
    PassPrediction::PassPeeper(328); // do-while
    Instruction *PI = Worklist.pop_back_val();
    for (User *U : PI->users()) {
      PassPrediction::PassPeeper(329); // for-range
      ...
      case Instruction::AddrSpaceCast:
        PassPrediction::PassPeeper(330); // case

      case Instruction::BitCast:
        PassPrediction::PassPeeper(331); // case

      case Instruction::GetElementPtr:
        PassPrediction::PassPeeper(332); // case
      ...
      case Instruction::ICmp:
        PassPrediction::PassPeeper(333); // case
        {
      ...
      case Instruction::Call:
        PassPrediction::PassPeeper(336); // case

        // Ignore no-op and store intrinsics.
        if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
          PassPrediction::PassPeeper(337); // if
          switch (II->getIntrinsicID()) {
          default:
            return false;

          case Intrinsic::memmove:
            PassPrediction::PassPeeper(338); // case

          case Intrinsic::memcpy:
            PassPrediction::PassPeeper(339); // case

          case Intrinsic::memset:
            PassPrediction::PassPeeper(340); // case
            {
              MemIntrinsic *MI = cast<MemIntrinsic>(II);
              if (MI->isVolatile() || MI->getRawDest() != PI) {
                PassPrediction::PassPeeper(341); // if
                return false;
              }
              LLVM_FALLTHROUGH;
            }
          case Intrinsic::dbg_declare:
            PassPrediction::PassPeeper(342); // case

          case Intrinsic::dbg_value:
            PassPrediction::PassPeeper(343); // case

          case Intrinsic::invariant_start:
            PassPrediction::PassPeeper(344); // case

          case Intrinsic::invariant_end:
            PassPrediction::PassPeeper(345); // case

          case Intrinsic::lifetime_start:
            PassPrediction::PassPeeper(346); // case

          case Intrinsic::lifetime_end:
            PassPrediction::PassPeeper(347); // case

          case Intrinsic::objectsize:
            PassPrediction::PassPeeper(348); // case
        ...
        if (isFreeCall(I, TLI)) {
          PassPrediction::PassPeeper(349); // if
          Users.emplace_back(I);
          continue;
        }
        return false;
      ...
      case Instruction::Store:
        PassPrediction::PassPeeper(350); // case
        {
          StoreInst *SI = cast<StoreInst>(I);
          if (SI->isVolatile() || SI->getPointerOperand() != PI) {
            PassPrediction::PassPeeper(351); // if
            return false;
          }
          Users.emplace_back(I);
          continue;
...
Instruction *InstCombiner::visitAllocSite(Instruction &MI) {
  ...
  if (isAllocSiteRemovable(&MI, Users, &TLI)) {
    PassPrediction::PassPeeper(352); // if
    for (unsigned i = 0, e = Users.size(); i != e; ++i) {
      // Lowering all @llvm.objectsize calls first because they may
      // use a bitcast/GEP of the alloca we are removing.
      PassPrediction::PassPeeper(353); // for
      if (!Users[i]) {
        PassPrediction::PassPeeper(354); // if
        continue;
      }
      ...
      if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)) {
        PassPrediction::PassPeeper(355); // if
        if (II->getIntrinsicID() == Intrinsic::objectsize) {
          PassPrediction::PassPeeper(356); // if
    ...
    for (unsigned i = 0, e = Users.size(); i != e; ++i) {
      PassPrediction::PassPeeper(357); // for
      if (!Users[i]) {
        PassPrediction::PassPeeper(358); // if
        continue;
      }
      ...
      if (ICmpInst *C = dyn_cast<ICmpInst>(I)) {
        PassPrediction::PassPeeper(359); // if
        replaceInstUsesWith(*C,
                            ConstantInt::get(Type::getInt1Ty(C->getContext()),
                                             C->isFalseWhenEqual()));
      } else if (isa<BitCastInst>(I) || isa<GetElementPtrInst>(I) ||
                 isa<AddrSpaceCastInst>(I)) {
        PassPrediction::PassPeeper(360); // if
    ...
    if (InvokeInst *II = dyn_cast<InvokeInst>(&MI)) {
      ...
      PassPrediction::PassPeeper(361); // if
...
static Instruction *tryToMoveFreeBeforeNullTest(CallInst &FI) {
  ...
  if (!PredBB) {
    PassPrediction::PassPeeper(362); // if
    return nullptr;
  }
  ...
  if (FreeInstrBB->size() != 2) {
    PassPrediction::PassPeeper(363); // if
    return nullptr;
  }
  BasicBlock *SuccBB;
  if (!match(FreeInstrBB->getTerminator(), m_UnconditionalBr(SuccBB))) {
    PassPrediction::PassPeeper(364); // if
    return nullptr;
  }
  ...
  if (!match(TI,
    ...
    PassPrediction::PassPeeper(365); // if
    return nullptr;
  }
  if (Pred != ICmpInst::ICMP_EQ && Pred != ICmpInst::ICMP_NE) {
    PassPrediction::PassPeeper(366); // if
    return nullptr;
  }

  // Validate constraint #3: Ensure the null case just falls through.
  if (SuccBB != (Pred == ICmpInst::ICMP_EQ ? TrueBB : FalseBB)) {
    PassPrediction::PassPeeper(367); // if
    return nullptr;
  }
...
Instruction *InstCombiner::visitFree(CallInst &FI) {
  ...
  if (isa<UndefValue>(Op)) {
    ...
    PassPrediction::PassPeeper(368); // if
  ...
  if (isa<ConstantPointerNull>(Op)) {
    PassPrediction::PassPeeper(369); // if
    return eraseInstFromFunction(FI);
  }
  ...
  // free(foo);
  if (MinimizeSize) {
    PassPrediction::PassPeeper(370); // if
    if (Instruction *I = tryToMoveFreeBeforeNullTest(FI)) {
      PassPrediction::PassPeeper(371); // if
      return I;
    }
  }
...
Instruction *InstCombiner::visitReturnInst(ReturnInst &RI) {
  if (RI.getNumOperands() == 0) {    // ret void
    PassPrediction::PassPeeper(372); // if
    return nullptr;
  }
  ...
  if (!VTy->isIntegerTy()) {
    PassPrediction::PassPeeper(373); // if
    return nullptr;
  }
  ...
  if (Known.isConstant()) {
    PassPrediction::PassPeeper(374); // if
...
Instruction *InstCombiner::visitBranchInst(BranchInst &BI) {
  ...
  if (match(&BI, m_Br(m_Not(m_Value(X)), TrueDest, FalseDest)) &&
    ...
    PassPrediction::PassPeeper(375); // if
    BI.setCondition(X);
    BI.swapSuccessors();
    return &BI;
  }
  ...
  if (BI.isConditional() && BI.getSuccessor(0) == BI.getSuccessor(1) &&
    ...
    PassPrediction::PassPeeper(376); // if
  ...
  if (match(&BI, m_Br(m_OneUse(m_Cmp(Pred, m_Value(), m_Value())), TrueDest,
    ...
    // Swap destinations and condition.
    PassPrediction::PassPeeper(377); // if
...
Instruction *InstCombiner::visitSwitchInst(SwitchInst &SI) {
  ...
  if (match(Cond, m_Add(m_Value(Op0), m_ConstantInt(AddRHS)))) {
    ...
    PassPrediction::PassPeeper(378); // if
    for (auto Case : SI.cases()) {
      PassPrediction::PassPeeper(379); // for-range
  ...
  for (auto &C : SI.cases()) {
    PassPrediction::PassPeeper(380); // for-range
  ...
  if (NewWidth > 0 && NewWidth < Known.getBitWidth()) {
    PassPrediction::PassPeeper(381); // if
    ...
    for (auto Case : SI.cases()) {
      PassPrediction::PassPeeper(382); // for-range
...
Instruction *InstCombiner::visitExtractValueInst(ExtractValueInst &EV) {
  ...
  if (InsertValueInst *IV = dyn_cast<InsertValueInst>(Agg)) {
    ...
    for (exti = EV.idx_begin(), insi = IV->idx_begin(), exte = EV.idx_end(),
      ...
      if (*insi != *exti) {
        ...
        PassPrediction::PassPeeper(387); // if
    ...
    if (exti == exte && insi == inse) {
      ...
      // with "i32 42"
      PassPrediction::PassPeeper(388); // if
    ...
    if (exti == exte) {
      ...
      PassPrediction::PassPeeper(389); // if
    ...
    if (insi == inse) {
      ...
      PassPrediction::PassPeeper(390); // if
  ...
  if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Agg)) {
    ...
    // just get one value.
    PassPrediction::PassPeeper(391); // if
    if (II->hasOneUse()) {
      // Check if we're grabbing the overflow bit or the result of a 'with
      // overflow' intrinsic.  If it's the latter we can remove the intrinsic
      // and replace it with a traditional binary instruction.
      PassPrediction::PassPeeper(392); // if
      switch (II->getIntrinsicID()) {
      case Intrinsic::uadd_with_overflow:
        PassPrediction::PassPeeper(393); // case

      case Intrinsic::sadd_with_overflow:
        PassPrediction::PassPeeper(394); // case

        if (*EV.idx_begin() == 0) {        // Normal result.
          PassPrediction::PassPeeper(395); // if
        ...
        if (II->getIntrinsicID() == Intrinsic::uadd_with_overflow) {
          PassPrediction::PassPeeper(396); // if
          if (ConstantInt *CI = dyn_cast<ConstantInt>(II->getArgOperand(1))) {
            PassPrediction::PassPeeper(397); // if
            return new ICmpInst(ICmpInst::ICMP_UGT, II->getArgOperand(0),
                                ConstantExpr::getNot(CI));
          }
        }
        PassPrediction::PassPeeper(398); // break
        break;
      case Intrinsic::usub_with_overflow:
        PassPrediction::PassPeeper(399); // case

      case Intrinsic::ssub_with_overflow:
        PassPrediction::PassPeeper(400); // case

        if (*EV.idx_begin() == 0) {        // Normal result.
          PassPrediction::PassPeeper(401); // if
          Value *LHS = II->getArgOperand(0), *RHS = II->getArgOperand(1);
          replaceInstUsesWith(*II, UndefValue::get(II->getType()));
          eraseInstFromFunction(*II);
          return BinaryOperator::CreateSub(LHS, RHS);
        }
        PassPrediction::PassPeeper(402); // break
        break;
      case Intrinsic::umul_with_overflow:
        PassPrediction::PassPeeper(403); // case

      case Intrinsic::smul_with_overflow:
        PassPrediction::PassPeeper(404); // case

        if (*EV.idx_begin() == 0) {        // Normal result.
          PassPrediction::PassPeeper(405); // if
          Value *LHS = II->getArgOperand(0), *RHS = II->getArgOperand(1);
          replaceInstUsesWith(*II, UndefValue::get(II->getType()));
          eraseInstFromFunction(*II);
          return BinaryOperator::CreateMul(LHS, RHS);
        }
        PassPrediction::PassPeeper(406); // break
        break;
      default:
        PassPrediction::PassPeeper(407); // break
        break;
      }
    }
  }
  if (LoadInst *L = dyn_cast<LoadInst>(Agg)) {
    ...
    PassPrediction::PassPeeper(408); // if
    if (L->isSimple() && L->hasOneUse()) {
      // extractvalue has integer indices, getelementptr has Value*s. Convert.
      PassPrediction::PassPeeper(409); // if
      SmallVector<Value *, 4> Indices;
      // Prefix an i32 0 since we need the first element.
      Indices.push_back(Builder.getInt32(0));
      for (ExtractValueInst::idx_iterator I = EV.idx_begin(), E = EV.idx_end();
           I != E; ++I) {
        PassPrediction::PassPeeper(410); // for
...
static bool isCatchAll(EHPersonality Personality, Constant *TypeInfo) {
  ...
  case EHPersonality::GNU_C:
    PassPrediction::PassPeeper(411); // case

  case EHPersonality::GNU_C_SjLj:
    PassPrediction::PassPeeper(412); // case

  case EHPersonality::Rust:
    PassPrediction::PassPeeper(413); // case
  ...
  case EHPersonality::Unknown:
    PassPrediction::PassPeeper(414); // case

    return false;
  case EHPersonality::GNU_Ada:
    PassPrediction::PassPeeper(415); // case

    // While __gnat_all_others_value will match any Ada exception, it doesn't
    // match foreign exceptions (or didn't, before gcc-4.7).
    return false;
  case EHPersonality::GNU_CXX:
    PassPrediction::PassPeeper(416); // case

  case EHPersonality::GNU_CXX_SjLj:
    PassPrediction::PassPeeper(417); // case

  case EHPersonality::GNU_ObjC:
    PassPrediction::PassPeeper(418); // case

  case EHPersonality::MSVC_X86SEH:
    PassPrediction::PassPeeper(419); // case

  case EHPersonality::MSVC_Win64SEH:
    PassPrediction::PassPeeper(420); // case

  case EHPersonality::MSVC_CXX:
    PassPrediction::PassPeeper(421); // case

  case EHPersonality::CoreCLR:
    PassPrediction::PassPeeper(422); // case
...
Instruction *InstCombiner::visitLandingPadInst(LandingPadInst &LI) {
  ...
  for (unsigned i = 0, e = LI.getNumClauses(); i != e; ++i) {
    PassPrediction::PassPeeper(423); // for
    bool isLastClause = i + 1 == e;
    if (LI.isCatch(i)) {
      // A catch clause.
      PassPrediction::PassPeeper(424); // if
      ...
      if (AlreadyCaught.insert(TypeInfo).second) {
        ...
        PassPrediction::PassPeeper(425); // if
        NewClauses.push_back(CatchClause);
      } else {
        // Repeated catch clause - drop the redundant copy.
        PassPrediction::PassPeeper(426); // else
      ...
      if (isCatchAll(Personality, TypeInfo)) {
        PassPrediction::PassPeeper(427); // if
        if (!isLastClause) {
          PassPrediction::PassPeeper(428); // if
          MakeNewInstruction = true;
        }
        CleanupFlag = false;
        PassPrediction::PassPeeper(429); // break
        break;
      }
    } else {
      ...
      if (!NumTypeInfos) {
        PassPrediction::PassPeeper(430); // if
        NewClauses.push_back(FilterClause);
        if (!isLastClause) {
          PassPrediction::PassPeeper(431); // if
          MakeNewInstruction = true;
        }
        CleanupFlag = false;
        PassPrediction::PassPeeper(432); // break
        break;
      }
      ...
      if (isa<ConstantAggregateZero>(FilterClause)) {
        ...
        if (isCatchAll(Personality, TypeInfo)) {
          ...
          PassPrediction::PassPeeper(434); // if
          MakeNewInstruction = true;
          continue;
        }
        ...
        if (NumTypeInfos > 1) {
          PassPrediction::PassPeeper(435); // if
          MakeNewFilter = true;
        }
      } else {
        PassPrediction::PassPeeper(433); // else
        ...
        for (unsigned j = 0; j != NumTypeInfos; ++j) {
          PassPrediction::PassPeeper(436); // for
          Constant *Elt = Filter->getOperand(j);
          Constant *TypeInfo = Elt->stripPointerCasts();
          if (isCatchAll(Personality, TypeInfo)) {
            // This element is a catch-all.  Bail out, noting this fact.
            PassPrediction::PassPeeper(437); // if
            SawCatchAll = true;
            PassPrediction::PassPeeper(438); // break
            break;
          }
          ...
          if (SeenInFilter.insert(TypeInfo).second) {
            PassPrediction::PassPeeper(439); // if
        ...
        if (SawCatchAll) {
          // Throw the filter away.
          PassPrediction::PassPeeper(440); // if
          MakeNewInstruction = true;
          continue;
        }
        ...
        if (NewFilterElts.size() < NumTypeInfos) {
          PassPrediction::PassPeeper(441); // if
          MakeNewFilter = true;
        }
      }
      if (MakeNewFilter) {
        PassPrediction::PassPeeper(442); // if
        FilterType =
      ...
      if (MakeNewFilter && !NewFilterElts.size()) {
        ...
        CleanupFlag = false;
        PassPrediction::PassPeeper(443); // break
        break;
      }
    }
  }
  ...
  for (unsigned i = 0, e = NewClauses.size(); i + 1 < e;) {
    ...
    for (unsigned k = i; k + 1 < j; ++k) {
      PassPrediction::PassPeeper(447); // for
      if (shorter_filter(NewClauses[k + 1], NewClauses[k])) {
        ...
        PassPrediction::PassPeeper(448); // if
        std::stable_sort(NewClauses.begin() + i, NewClauses.begin() + j,
                         shorter_filter);
        MakeNewInstruction = true;
        PassPrediction::PassPeeper(449); // break
        break;
      }
    }
  ...
  for (unsigned i = 0; i + 1 < NewClauses.size(); ++i) {
    ...
    for (unsigned j = NewClauses.size() - 1; j != i; --j) {
      ...
      if (!FElts) {
        // Discard LFilter.
        PassPrediction::PassPeeper(454); // if
        NewClauses.erase(J);
        MakeNewInstruction = true;
      ...
      if (FElts > LElts) {
        // Move on to the next filter.
        PassPrediction::PassPeeper(455); // if
        continue;
      }
      ...
      if (isa<ConstantAggregateZero>(LFilter)) { // LFilter only contains zeros.
        ...
        PassPrediction::PassPeeper(456); // if
      ...
      if (isa<ConstantAggregateZero>(Filter)) { // Filter only contains zeros.
        ...
        for (unsigned l = 0; l != LElts; ++l) {
          PassPrediction::PassPeeper(457); // for
          if (LArray->getOperand(l)->isNullValue()) {
            // LFilter contains a zero - discard it.
            PassPrediction::PassPeeper(458); // if
            NewClauses.erase(J);
            MakeNewInstruction = true;
            PassPrediction::PassPeeper(459); // break
            break;
          }
        }
        // Move on to the next filter.
      ...
      for (unsigned f = 0; f != FElts; ++f) {
        PassPrediction::PassPeeper(460); // for
        Value *FTypeInfo = FArray->getOperand(f)->stripPointerCasts();
        AllFound = false;
        for (unsigned l = 0; l != LElts; ++l) {
          PassPrediction::PassPeeper(461); // for
          Value *LTypeInfo = LArray->getOperand(l)->stripPointerCasts();
          if (LTypeInfo == FTypeInfo) {
            PassPrediction::PassPeeper(462); // if
            AllFound = true;
            PassPrediction::PassPeeper(463); // break
            break;
          }
        }
        if (!AllFound) {
          PassPrediction::PassPeeper(464); // if
          break;
        }
      }
      if (AllFound) {
        // Discard LFilter.
        PassPrediction::PassPeeper(465); // if
        NewClauses.erase(J);
        MakeNewInstruction = true;
      }
      // Move on to the next filter.
  ...
  if (MakeNewInstruction) {
    PassPrediction::PassPeeper(466); // if
    LandingPadInst *NLI =
        LandingPadInst::Create(LI.getType(), NewClauses.size());
    for (unsigned i = 0, e = NewClauses.size(); i != e; ++i) {
      PassPrediction::PassPeeper(467); // for
    ...
    if (NewClauses.empty()) {
      PassPrediction::PassPeeper(468); // if
      CleanupFlag = true;
    }
...
static bool TryToSinkInstruction(Instruction *I, BasicBlock *DestBlock) {
  ...
  if (isa<PHINode>(I) || I->isEHPad() || I->mayHaveSideEffects() ||
    ...
    PassPrediction::PassPeeper(469); // if
    return false;
  }
  ...
  if (isa<AllocaInst>(I) &&
    ...
    PassPrediction::PassPeeper(470); // if
    return false;
  }

  // Do not sink into catchswitch blocks.
  if (isa<CatchSwitchInst>(DestBlock->getTerminator())) {
    PassPrediction::PassPeeper(471); // if
    return false;
  }

  // Do not sink convergent call instructions.
  if (auto *CI = dyn_cast<CallInst>(I)) {
    PassPrediction::PassPeeper(472); // if
    if (CI->isConvergent()) {
      PassPrediction::PassPeeper(473); // if
      return false;
    }
  }
  ...
  if (I->mayReadFromMemory()) {
    PassPrediction::PassPeeper(474); // if
    for (BasicBlock::iterator Scan = I->getIterator(),
                              E = I->getParent()->end();
         Scan != E; ++Scan) {
      PassPrediction::PassPeeper(475); // for
      if (Scan->mayWriteToMemory()) {
        PassPrediction::PassPeeper(476); // if
        return false;
      }
    }
  }
...
bool InstCombiner::run() {
  while (!Worklist.isEmpty()) {
    PassPrediction::PassPeeper(477); // while
    Instruction *I = Worklist.RemoveOne();
    if (I == nullptr) {
      PassPrediction::PassPeeper(478); // if
    ...
    if (!I->use_empty() &&
      ...
      PassPrediction::PassPeeper(479); // if
      if (Constant *C = ConstantFoldInstruction(I, DL, &TLI)) {
        ...
        ++NumConstProp;
        if (isInstructionTriviallyDead(I, &TLI)) {
          PassPrediction::PassPeeper(480); // if
          eraseInstFromFunction(*I);
        }
        MadeIRChange = true;
        continue;
    ...
    if (ExpensiveCombines && !I->use_empty() && Ty->isIntOrIntVectorTy()) {
      PassPrediction::PassPeeper(481); // if
      KnownBits Known = computeKnownBits(I, /*Depth*/ 0, I);
      if (Known.isConstant()) {
        PassPrediction::PassPeeper(482); // if
        ...
        ++NumConstProp;
        if (isInstructionTriviallyDead(I, &TLI)) {
          PassPrediction::PassPeeper(483); // if
          eraseInstFromFunction(*I);
        }
        MadeIRChange = true;
        continue;
    ...
    if (I->hasOneUse()) {
      PassPrediction::PassPeeper(484); // if
      ...
      if (PHINode *PN = dyn_cast<PHINode>(UserInst)) {
        PassPrediction::PassPeeper(485); // if
        UserParent = PN->getIncomingBlock(*I->use_begin());
      } else {
        PassPrediction::PassPeeper(486); // else
        UserParent = UserInst->getParent();
      }

      if (UserParent != BB) {
        PassPrediction::PassPeeper(487); // if
        bool UserIsSuccessor = false;
        // See if the user is one of our successors.
        for (succ_iterator SI = succ_begin(BB), E = succ_end(BB); SI != E;
             ++SI) {
          PassPrediction::PassPeeper(488); // for
          if (*SI == UserParent) {
            PassPrediction::PassPeeper(489); // if
            UserIsSuccessor = true;
            PassPrediction::PassPeeper(490); // break
            break;
          }
        }
        ...
        if (UserIsSuccessor && UserParent->getUniquePredecessor()) {
          ...
          PassPrediction::PassPeeper(491); // if
          if (TryToSinkInstruction(I, UserParent)) {
            ...
            for (Use &U : I->operands()) {
              PassPrediction::PassPeeper(492); // for-range
              if (Instruction *OpI = dyn_cast<Instruction>(U.get())) {
                PassPrediction::PassPeeper(493); // if
                Worklist.Add(OpI);
              }
            }
          }
    ...
    if (Instruction *Result = visit(*I)) {
      PassPrediction::PassPeeper(494); // if
      ++NumCombined;
      ...
      if (Result != I) {
        ...
        if (I->getDebugLoc()) {
          PassPrediction::PassPeeper(495); // if
        ...
        if (!isa<PHINode>(Result) && isa<PHINode>(InsertPos)) {
          PassPrediction::PassPeeper(496); // if
        ...
        // if so, remove it.
        if (isInstructionTriviallyDead(I, &TLI)) {
          PassPrediction::PassPeeper(497); // if
          eraseInstFromFunction(*I);
        } else {
          PassPrediction::PassPeeper(498); // else
...
static bool AddReachableCodeToWorklist(BasicBlock *BB, const DataLayout &DL,
  ...
  do {
    PassPrediction::PassPeeper(499); // do-while
    BB = Worklist.pop_back_val();

    // We have now visited this block!  If we've already been here, ignore it.
    if (!Visited.insert(BB).second) {
      PassPrediction::PassPeeper(500); // if
      continue;
    }

    for (BasicBlock::iterator BBI = BB->begin(), E = BB->end(); BBI != E;) {
      PassPrediction::PassPeeper(501); // for
      Instruction *Inst = &*BBI++;

      // DCE instruction if trivially dead.
      if (isInstructionTriviallyDead(Inst, TLI)) {
        PassPrediction::PassPeeper(502); // if
        ++NumDeadInst;
      ...
      if (!Inst->use_empty() &&
        ...
        PassPrediction::PassPeeper(503); // if
        if (Constant *C = ConstantFoldInstruction(Inst, DL, TLI)) {
          ...
          ++NumConstProp;
          if (isInstructionTriviallyDead(Inst, TLI)) {
            PassPrediction::PassPeeper(504); // if
            Inst->eraseFromParent();
          }
          MadeIRChange = true;
          continue;
      ...
      for (Use &U : Inst->operands()) {
        PassPrediction::PassPeeper(505); // for-range
        if (!isa<ConstantVector>(U) && !isa<ConstantExpr>(U)) {
          PassPrediction::PassPeeper(506); // if
          continue;
        }
        ...
        if (!FoldRes) {
          PassPrediction::PassPeeper(507); // if
          FoldRes = ConstantFoldConstant(C, DL, TLI);
        }
        if (!FoldRes) {
          PassPrediction::PassPeeper(508); // if
          FoldRes = C;
        }
      ...
      if (!isa<DbgInfoIntrinsic>(Inst)) {
        PassPrediction::PassPeeper(509); // if
    ...
    if (BranchInst *BI = dyn_cast<BranchInst>(TI)) {
      PassPrediction::PassPeeper(510); // if
      if (BI->isConditional() && isa<ConstantInt>(BI->getCondition())) {
        PassPrediction::PassPeeper(511); // if
      ...
      PassPrediction::PassPeeper(512); // if
      if (ConstantInt *Cond = dyn_cast<ConstantInt>(SI->getCondition())) {
        PassPrediction::PassPeeper(513); // if
    ...
    for (BasicBlock *SuccBB : TI->successors()) {
      PassPrediction::PassPeeper(514); // for-range
...
static bool prepareICWorklistFromFunction(Function &F, const DataLayout &DL,
  ...
  for (BasicBlock &BB : F) {
    PassPrediction::PassPeeper(515); // for-range
    if (Visited.count(&BB)) {
      PassPrediction::PassPeeper(516); // if
      continue;
    }
...
static bool combineInstructionsOverFunction(
  ...
  IRBuilder<TargetFolder, IRBuilderCallbackInserter> Builder(
      ...
      IRBuilderCallbackInserter([&Worklist, &AC](Instruction *I) {
        ...
        if (match(I, m_Intrinsic<Intrinsic::assume>())) {
          PassPrediction::PassPeeper(517); // if
  ...
  int Iteration = 0;
  for (;;) {
    PassPrediction::PassPeeper(518); // for
    ++Iteration;
    ...
    if (!IC.run()) {
      PassPrediction::PassPeeper(519); // if
      break;
    }
  }
...
PreservedAnalyses InstCombinePass::run(Function &F,
  ...
  if (!combineInstructionsOverFunction(F, Worklist, nullptr, AC, TLI, DT,
    ...
    PassPrediction::PassPeeper(520); // if
...
bool InstructionCombiningPass::runOnFunction(Function &F) {
  if (skipFunction(F)) {
    PassPrediction::PassPeeper(521); // if
    return false;
  }
